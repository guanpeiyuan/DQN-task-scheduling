...
def EDF_load_balance(task_arrive_list, total_timeslot, core_number, max_lenth_of_task_pool):
    ...
    return execute_sequence.__len__(), accumulate_utility, sum(
        utility_sequence), task_decline_information, task_decline_id
...
def global_PLA(task_arrive_list, total_timeslot, core_number, times, max_lenth_of_task_pool):
    ...
    return len(execute_sequence), accumulate_utility, sum(
        utility_sequence), task_decline_information, error_flag, task_wait_time
...
def global_EDF(task_arrive_list, total_timeslot, core_number, times, max_lenth_of_task_pool):
    ...
    for timeslot in range(total_timeslot):
        ...
        if timeslot >= 30 and 0 < idle_cpu < len(task_pool):
            ...
            print("保存第{}条经验".format(len(agent.memory)+1))
            # # 对输入数据进行归一化(标准化？)
            # 记忆先前的状态，行为，回报与下一个状态   #  将当前状态，于执行动作后的状态，存入经验池

                # agent.save_exp(preprocessing.scale(state).reshape((1, 25, 6, 1)), _action, _reward,
                #                preprocessing.scale(_next_state).reshape((1, 25, 6, 1)), _done)
            if _done == False:
...
def main():
    ...
    # f = open('d:\\data_1(1W)_newlabel.pkl', 'rb')
    #
    # data = pickle.load(f)
    #
    # f.close()
    #
    # f = open('d:\\label_1(1W)_newlabel.pkl', 'rb')
    #
    # label = pickle.load(f)
    #
    # f.close()
    ...
    #
    #     label[i] = normalization(label[i])
    #
    #     print("第{}个标签完成正则化".format(i))

    # agent.model.fit(data[:,:,1:5,:],label,batch_size=64,epochs=100,verbose=1)
    #
    # agent.model.save('gpy.h5')
    #
    # print("训练结束")
...
class DQNAgent(object):
    def __init__(self, _env):
        ...
        # 控制训练的随机干涉
        self.epsilon = 0.7  # 随机干涉阈值 该值会随着训练减少
        self.epsilon_decay = 0.995  # 每次随机衰减0.005
    ...
    def _build_model(self):
        ...
        model.add(tf.keras.layers.Conv2D(4, (2, 2), activation='relu', input_shape=(25, 4, 1)))
        # 第2层卷积，卷积核大小为2*2，32个
    ...
    def train_exp(self):
        ...
        fit_state = np.zeros(shape=(1,25,6,1))

        fit_lable = np.zeros(shape=(1,175))
        ...
        for i in range(len(self.memory)):
            ...
            sample_state = np.expand_dims(_state,2)

            sample_state = np.expand_dims(sample_state,0)

            sample_next_state = np.expand_dims(_next_state,2)

            sample_next_state = np.expand_dims(sample_next_state,0)

            # print("预测next state能带来的收益：{}".format(np.max(self.model.predict(sample_next_state[:, :, 1:5, :])[0])))

            if _reward==-100:
                y_reward = -100
            else:

                action_vector = self.model.predict(sample_next_state[:, :, 1:5, :])[0]

                action_position = np.argmax(action_vector)

                y_reward = _reward + self.gamma * action_vector[action_position]

                print("")

            # print("本次state的reward预测值：{}".format(np.max(self.model.predict(sample_state[:, :, 1:5, :])[0])))
            print("")
            ...
            _y = self.model.predict(sample_state[:, :, 1:5, :])[0]

            _y[_action] = y_reward

            # 选定动作的奖励已更新，现在处理大部分明显错误的动作选项

            for i in range(len(_y)):

                agent.env.env.cpu = copy.deepcopy(_state[10:15,:])
                agent.env.env.task_pool = copy.deepcopy(_state[0:10,:])
                agent.env.env.arrived_task = copy.deepcopy(_state[15:,:])

                if i != _action:

                    label_next_state, label_reward, label_done, label_ = env.step(i)
                    ...
                    if label_reward < 0:
                        ...
                        sample_label_next_state = np.expand_dims(label_next_state, 2)

                        sample_label_next_state = np.expand_dims(sample_label_next_state, 0)

                        action_vector = self.model.predict(sample_label_next_state[:, :, 1:5, :])[0]

                        action_position = np.argmax(action_vector)

                        _y[i] = label_reward + self.gamma * action_vector[action_position]



            # _y = normalization(_y)

            fit_state = np.vstack((fit_state,sample_state))

            fit_lable = np.vstack((fit_lable,_y))

        fit_lable = np.delete(fit_lable,0,0)

        fit_state = np.delete(fit_state,0,0)

        f = open('d:\\data_1(2W)_newlabel.pkl', 'wb')

        pickle.dump(fit_state,f,0)

        f.close()

        f = open('d:\\label_1(2W)_newlabel.pkl', 'wb')

        pickle.dump(fit_lable,f,0)
        ...
        for i in range(10):
            ...
            np.random.shuffle(fit_state)

            self.model.fit(fit_state[:,:,1:5,:],fit_lable,batch_size=32,epochs=200,verbose=1)
    ...
    def act(self, _state):  # 返回回报最大的奖励值，或 随机数
        ...
        if np.random.rand() >= self.epsilon:
            ...
            action = np.random.randint(0, 175)
            ...
            sample_state = np.expand_dims(np.expand_dims(_state,0),3)
            action = np.argmax(self.model.predict(sample_state[:, :, 1:5, :])[0])
            ...
            #         self.task_pool_test = np.vstack((self.task_pool_test,
            #                                          [-1 + random.gauss(mu, sigma), -1 + random.gauss(mu, sigma),
            #                                           -1 + random.gauss(mu, sigma), -1 + random.gauss(mu, sigma),
            #                                           -1 + random.gauss(mu, sigma), -1 + random.gauss(mu, sigma)]))
            ...
            #                     self.task_pool_test[item] = [-1 + random.gauss(mu, sigma), -1 + random.gauss(mu, sigma),
            #                                                  -1 + random.gauss(mu, sigma), -1 + random.gauss(mu, sigma),
            #                                                  -1 + random.gauss(mu, sigma), -1 + random.gauss(mu, sigma)]
            ...
            #             state_test = np.vstack((state_test, [-1 + random.gauss(mu, sigma), -1 + random.gauss(mu, sigma),
            #                                                  -1 + random.gauss(mu, sigma), -1 + random.gauss(mu, sigma),
        ...
        print("候选action编号：{}".format(action))
...
if __name__ == '__main__':
    ...
    #         if item == 'gpy.h5':
    #             agent.model = tf.keras.models.load_model('gpy.h5')
